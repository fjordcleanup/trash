...
import { APP_DOMAIN_CERTIFICATE_STACK_NAME } from './stackName.ts'

/**
 * Certificates need to be in us-east-1 for CloudFront distributions.
 */
export class AppCustomDomainCertificateStack extends Stack {
	public constructor(
		parent: App,
		{
			baseDomainName,
		...
		super(parent, APP_DOMAIN_CERTIFICATE_STACK_NAME, {
			env: {
				...env,
		...
		// Create certificate for the app subdomain
		const certificate = new CertificateManager.Certificate(
			this,
			'appCertificate',
			{
				domainName: `trash.${baseDomainName}`,
				validation:
	...
	public static parameterName(): string {
		return `/${APP_DOMAIN_CERTIFICATE_STACK_NAME}/certificateArn`
	}
}
