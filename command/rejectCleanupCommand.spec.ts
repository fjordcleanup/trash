...
import { rejectCleanupCommand } from './rejectCleanupCommand.ts'

void describe('rejectCleanupCommand()', () => {
	void it('should reject a cleanup and persist it', async () => {
		...
		const reject = rejectCleanupCommand(findMock, persistMock)

		const adminActorId = testActor()
		const cleanup = await reject(existingCleanup.$meta.id, v1, adminActorId)
		...
		const expectedEvent: Partial<CleanupRejectedEvent> = {
	...
	void it('should throw an error if the cleanup version does not match', async () => {
		...
		await assert.rejects(
			async () => {
				await reject(existingCleanup.$meta.id, inc(v1), testActor())
			},
			{
				name: 'ConflictError',
